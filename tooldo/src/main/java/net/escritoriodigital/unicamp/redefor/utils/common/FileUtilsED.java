 ...
 *
 */
public class FileUtilsED {

	/**
	 * Cria o diretório no filesystem do sistema caso não exista
	 *
	 * @param path
	 * @throws IOException
	 */
	public static void createFolderIfNotExists(String path) throws IOException {
		...
		if (!folder.exists()) {
			if (!folder.mkdirs()) {
				throw new IOException("Não foi possível criar a pasta.");
			}
		}
	}
	 ...
	 * @return
	 */
	public static byte[] getBytesFromBufferedImage(BufferedImage bi,
			String format) {
